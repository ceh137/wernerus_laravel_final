...
class ManagerTable extends Component
{
    ...
    public function mount() {
        ...
        $payers_debt =  $payers_debt->toArray();
        $this->customers = $payers_debt;
    ...
    public function applyDateFilter() {
        ...
        if ($this->from && $this->till) {
            ...
            foreach ($payers as $payer) {
                ...
                foreach ($payer->orders()->whereBetween('time_to_order', [$this->from, $this->till]) as $order) {
                    ...
                    if ($payer instanceof Customer) {
                        ...
                        foreach ($fields as $field => $price) {
                            if ($order->$who == $order->who_pays->$field) {
                                if (!empty($paid[$who_payer][$field]) && isset($paid[$who_payer][$field]) && $paid[$who_payer][$field]) {
                                    $revenue += $order->order_prices->$price ? : 0;
                                } else {
                                    $revenue += $order->order_prices->$price ? : 0;
//                                    $debt += $order->order_prices->$price ? : 0;
                                }
